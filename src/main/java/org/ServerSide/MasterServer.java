...
public class MasterServer {
    ...
    void connectWorker(ObjectOutputStream newOut, ObjectInputStream newIn) throws IOException {
        int assignedId = -1;
        ...
        for (Map.Entry<Integer, Pair<ObjectOutputStream, ObjectInputStream>> e : worker_streams.entrySet()) {
            ...
            try {
                ...
                Boolean alive = (Boolean) monitor.getResult(3_000);
                if (!alive) {
                    assignedId = workerId;
...
//            replicated_worker_handlers.remove(assignedId);
        }

        worker_streams.put(assignedId, new Pair<>(newOut, newIn));

        try {
            synchronized (newOut) {
                newOut.reset();
                newOut.writeInt(assignedId);
                newOut.writeObject(worker_shop_map.get(assignedId));
                newOut.flush();
            }
        } catch (IOException e) {
...
//        rh.clearData();
//        rh.setId(assignedId);
//        rh.setMainId(assignedId);
//        replicated_worker_handlers.put(assignedId, rh);
            ...
            while (true) {
                ...
                (new Thread(() -> {
                    ...
                    if (connection_type == ConnectionType.CLIENT) {
                        try {
                            t = new ClientRequestHandler(server_socket, f_out, f_in);
                        } catch (IOException e) {
                            throw new RuntimeException(e);
                        }
                    }
